-- LC-33: dealer-initiated gameplay RPCs now return the inserted session_events
-- row so the client can apply the event optimistically without waiting for the
-- Supabase Realtime echo (which is unreliably delivered back to the writing
-- client in the self-hosted setup). Supersedes the return types in 017 and the
-- bodies in 018 for multi-level proficiency.
